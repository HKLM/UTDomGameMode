...
#include "StatNames.h"
#include "UTDomStat.h"
#include "SiegeScoreboard.h"

USiegeScoreboard::USiegeScoreboard(const class FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	NumPages = 2;
}

void USiegeScoreboard::DrawPlayerStats(AUTPlayerState* PS, float DeltaTime, float& YPos, float XOffset, float ScoreWidth, float MaxHeight, const FStatsFontInfo& StatsFontInfo)
{
	...
	int32 SiegePointHeldPt = PS->GetStatsValue(NAME_ControlPointHeldPoints);
	int32 RegularKills = PS->Kills - SiegePointHeldPt;
	...
	DrawStatsLine(NSLOCTEXT("SiegeScoreboard", "ControlPointHeldPoints", " - Control Point Score"), SiegePointHeldPt, PS->GetStatsValue(NAME_ControlPointHeldPoints), DeltaTime, XOffset, YPos, TinyFontInfo, ScoreWidth);
	...
	DrawStatsLine(NSLOCTEXT("SiegeScoreboard", "ControlPointCaps", "Control Point Captures"), PS->GetStatsValue(NAME_ControlPointCaps), -1, DeltaTime, XOffset, YPos, StatsFontInfo, ScoreWidth);
	FText ClockString = UTHUDOwner->ConvertTime(FText::GetEmpty(), FText::GetEmpty(), PS->GetStatsValue(NAME_ControlPointHeldTime), false);
	DrawTextStatsLine(NSLOCTEXT("SiegeScoreboard", "ControlPointHeldTime", "Control Point Held Time"), ClockString.ToString(), "", DeltaTime, XOffset, YPos, StatsFontInfo, ScoreWidth, 0);
...
void USiegeScoreboard::DrawTeamStats(float DeltaTime, float& YPos, float XOffset, float ScoreWidth, float PageBottom, const FStatsFontInfo& StatsFontInfo)
{
	// draw team stats
	...
	DrawPlayerStatsLine(NSLOCTEXT("SiegeScoreboard", "TopCaps", "Top Conrtol Point Captures"), TopTeamCapRed, TopTeamCapBlue, DeltaTime, XOffset, YPos, StatsFontInfo, ScoreWidth, 0);
