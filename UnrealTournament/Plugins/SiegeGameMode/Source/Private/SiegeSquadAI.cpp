...
ASiegeSquadAI::ASiegeSquadAI(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bGotObjectives = false;
}

void ASiegeSquadAI::Initialize(AUTTeamInfo* InTeam, FName InOrders)
{
	Super::Initialize(InTeam, InOrders);

	for (TActorIterator<AUTGameObjective> It(GetWorld()); It; ++It)
	{
		ASiegePoint* SP = Cast<ASiegePoint>(It->GetCarriedObject());
		if (SP != NULL)
		{
...
void ASiegeSquadAI::BeginPlay()
{
	Super::BeginPlay();
	FindControlPoints();
}

bool ASiegeSquadAI::CheckSquadObjectives(AUTBot* B)
{
	...
	{
		if (GameObjective != NULL && GameObjective->GetCarriedObject()->GetTeamNum() != B->GetTeamNum())
		{
		...
		else if (GameObjective != NULL && GameObjective->GetCarriedObject()->GetTeamNum() == B->GetTeamNum() && GetWorld()->GetTimeSeconds() - 10.0f > LastObjectiveChange)
		{
	...
	{
		if (CurrentOrders == NAME_Defend  && GameObjective->GetCarriedObject()->GetTeamNum() == B->GetTeamNum() && GetWorld()->GetTimeSeconds() - 120.0f > LastObjectiveChange)
		{
			tmpObjective = GetNewObjective(GameObjective, B);
			if (tmpObjective != NULL)
			{
		...
		else if (CurrentOrders == NAME_Backup && GameObjective != NULL && GameObjective->GetCarriedObject()->GetTeamNum() == B->GetTeamNum() && GetWorld()->GetTimeSeconds() - 30.0f > LastObjectiveChange)
		{
			tmpObjective = GetNewObjective(GameObjective, B);
			if (tmpObjective != NULL)
			{
...
bool ASiegeSquadAI::IsNearEnemyBase(const FVector& TestLoc)
{
...
AUTGameObjective* ASiegeSquadAI::GetNewObjective(AActor* OldObjective, AUTBot* B)
{
	...
	int8 randomObjective = FMath::RandRange(0, 1);
	int8 LotteryNumber = FMath::RandRange(0, 1);
	...
	if (B != NULL)
	{
		if (TheOldObjective != NULL)
		{
			for (uint8 i = 0; i < GameControlPoints.Num(); i++)
			{
				...
				if ((TheOldObjective != GameControlPoints[i]
					&& GameControlPoints[i]->GetCarriedObject()->GetTeamNum() != B->GetTeamNum()
			...
			if (BestObjective == NULL)
			{
				...
				for (uint8 j = 0; j < GameControlPoints.Num(); j++)
				{
					if (TheOldObjective != GameControlPoints[j]
						&& GameControlPoints[j]->GetCarriedObject()->GetTeamNum() != B->GetTeamNum())
					{
		...
		if (BestObjective == NULL)
		{
			...
			{
				// Find a near by control point that our team does not control
				if (GameControlPoints[p]->GetCarriedObject()->GetTeamNum() != B->GetTeamNum()
...
void ASiegeSquadAI::FindControlPoints()
{
...
void ASiegeSquadAI::NotifyObjectiveEvent(AActor* InObjective, AController* InstigatedBy, FName EventName)
{
...
AUTGameObjective* ASiegeSquadAI::GetNearestObjective(AUTBot* InstigatedBy, bool bOnlyNotControlled)
{
	AUTGameObjective* BestObjective = NULL;
	float bestDistance = 100000.0f;

	FSiegeAITargets f;
	TArray<FSiegeAITargets> CP;

	for (uint8 i = 0; i < GameControlPoints.Num(); i++)
	{
		if (bOnlyNotControlled && GameControlPoints[i]->GetCarriedObject()->GetTeamNum() != InstigatedBy->GetTeamNum()
			|| !bOnlyNotControlled)
		{
