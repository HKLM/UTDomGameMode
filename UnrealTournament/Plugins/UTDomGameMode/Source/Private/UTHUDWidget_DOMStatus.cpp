	...
	: Super(ObjectInitializer)
{
	...
	static ConstructorHelpers::FObjectFinder<UTexture2D> HUDArrowtex(TEXT("Texture2D'/Game/RestrictedAssets/UI/HUDAtlas01.HUDAtlas01'"));
...
void UUTHUDWidget_DOMStatus::Draw_Implementation(float DeltaTime)
{
	...
	IconSize = FMath::Clamp(IcoMulti * RenderScale, 50.0f, 60.0f);
	...
	if (!bControlPointInitialized)
	{
		...
		for (int i = 0; i < CtrlPoints.Num(); i++)
		{
			...
			CtrlPoints[i].DomArrowDir.RenderColor = FLinearColor::Yellow;
	...
	for (int i = 0; i < CtrlPoints.Num(); i++)
	{
		...
		DrawTexture(DomTeamIconTexture[nTeam], POS.X, POS.Y, IconSize, IconSize, 2, 2, 255, 255, UTHUDOwner->HUDWidgetSlateOpacity, TeamColors[nTeam]);
		FString work;
		float px = POS.X;
		float py = POS.Y;
...
{
	if (DomTeamIconTexture.IsValidIndex(TeamIndex))
	{
		return DomTeamIconTexture[TeamIndex];
	}
	else
	{
		return DomTeamIconTexture[4];
	}
}
